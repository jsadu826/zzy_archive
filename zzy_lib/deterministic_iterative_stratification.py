#NOTE - copied from [scikit-multilearn 0.2.0]/model_selection/iterative_stratification.py
# -*- coding: utf-8 -*-
...
def deterministic_iterative_train_test_split(X, y, test_size): #NOTE - renamed
    ...
    stratifier = DeterministicIterativeStratification(n_splits=2, order=2, sample_distribution_per_fold=[test_size, 1.0-test_size])
...
def _fold_tie_break(desired_samples_per_fold, M):
    ...
    if len(M) == 1:
        ...
        return M_prim[0]  #NOTE - so that the tie break is now deterministic
...
class DeterministicIterativeStratification(_BaseKFold): #NOTE - renamed
    ...
    def __init__(self, n_splits=3, order=1, sample_distribution_per_fold = None, random_state=None):
        self.order = order
        super(
            DeterministicIterativeStratification,
